...
days = "BETWEEN '2021-11-21 16:00:00' and '2022-01-04 16:00:00' "
print(days)
...
with open(filename, 'r', encoding="UTF-8") as f:
    lists = json.load(f, encoding='UTF-8')

# for index,row in dv.iterrows():
#     rq = str(row["周期"])
#     nian = rq.split("-")[0].split(".")[0]
#     if nian=="10" or nian=="11" or nian=="12" or nian=="01" or nian=="02":
#         nian="旺季"
#     else:
#         nian = "淡季"
#     print(nian)
for k in lists:
    dv.loc[(dv.channel_code == k["渠道"]) & (dv.des == k["国家"]), '起飞上期数据'] = k["起飞-落地"]
    dv.loc[(dv.channel_code == k["渠道"]) & (dv.des == k["国家"]), '落地上期数据'] = k["落地-清关"]
    dv.loc[(dv.channel_code == k["渠道"]) & (dv.des == k["国家"]), '清关上期数据'] = k["清关-交付"]



dv.to_excel(r'F:/周报/wish/周维度.xlsx',index=False)
